...
namespace CoreWeb.Repository
{
    public class BuddyRepository : IRepository<Buddy, BuddyLookup>
    {
        ...
        {
            var factory = connectionFactory.Get();
            using (var connection = await factory.CreateConnectionAsync())
            {
                using (var channel = await connection.CreateChannelAsync())
                {
        ...
        public async Task SendDeleteEventAsync(Profile from, Profile to)
        {
            await SendBuddyEventAsync("del_buddy", from, to);
        }
        private async Task SendBuddyEventAsync(String type, Profile from, Profile to)
        {
            ConnectionFactory factory = connectionFactory.Get();
            using (var connection = await factory.CreateConnectionAsync())
            {
